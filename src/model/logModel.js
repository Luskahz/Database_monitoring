...
export async function listPeriodoDispFromTable(metadados) {
  const { tabela, coluna_data } = metadados;

  if (!coluna_data) throw new Error("Coluna de data não especificada");

  try {
    const [result] = await db.query(`SELECT \`${coluna_data}\` FROM \`${tabela}\``);
    return result||[];
  } catch (error) {
...
export async function deletePeriodoDispFromTable(tabela, colunaData, mes, ano){
  try {
    ...
    console.log(`\x1b[33mPeríodo deletado: ${mes}/${ano} da tabela ${tabela}\x1b[0m`);
    return result;
  } catch (error) {
...
export async function insertValidator(list, dataJson, colunaData) {
  try {
    ...
    const datasCsv = new Set(
      dataJson.map(linha => new Date(linha[colunaData]).toISOString().split("T")[0])
    );
